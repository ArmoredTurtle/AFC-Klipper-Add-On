...
class afc:
    def __init__(self, config):
        ...
        if self.spoolman:
            ...
            self.spoolman_ip = config.get('spoolman_ip')
            self.spoolman_port = config.get('spoolman_port')
            response = urlopen(f'http://{self.spoolman_ip}:{self.spoolman_port}/api/v1/filament')

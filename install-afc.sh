...
function clone_repo() {
  local afc_dir_name afc_base_name current_branch
  ...
  if [ ! -d "${AFC_PATH}" ]; then
    ...
    if [ "$current_branch" != "$AFC_BRANCH" ]; then
      echo "Switching to branch ${AFC_BRANCH}"
      git checkout "$AFC_BRANCH"
    fi
    cd -
  fi
...
function unlink_extensions() {
  ...
  echo -e -n "\nEnsure you remove the configuration from your moonraker.conf file\n"
...
while getopts "k:s:c:b:uh" arg; do
  case ${arg} in
